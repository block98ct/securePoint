...
module.exports = {
    ...
    fetchUserByActToken: async (act_token) => {
        return db.query("select * from user_register where actToken = ?", [act_token]);
    },
    updateUserByActToken: async (id) => {
        return db.query(
            `Update user_register set isVerified = 1 where id = ?`, [id]);
    },
    ...
    setUSerStatus: async(status, id)=>{
        return db.query(`UPDATE user_register SET status  = ? WHERE id = ?`, [status, id]);
    },
    
    setUserLoginStatus: async(currtime, email)=>{
        return db.query(`UPDATE user_register SET lastLogin  = ? WHERE email = ?`, [currtime, email]);
    },
    ...
    backgroundInformation: async (backGroundInfoData) => {
        return db.query("insert into background_information_form set ?", [backGroundInfoData]);
    },
    vehicleExperience: async (vehicleExpData) => {
        return db.query("insert into vehicle_experience_form set ?", [vehicleExpData]);
    },
    ...
    getBackgroundInformation: async (userId) => {
        return db.query(`SELECT * FROM background_information_form WHERE userId = ?`, [userId]);
    },
    getVehicleExperience: async (userId) => {
        return db.query(`SELECT * FROM vehicle_experience_form WHERE userId = ?`, [userId]);
    },

    getPersonalDetails: async(userId)=>{
        return db.query(`SELECT * FROM personal_details WHERE userId = ?`, [userId]);
    },
    ...
    getAcknowledgeDetails: async(userId)=>{
        return db.query(`SELECT * FROM acknowledge_details WHERE userId = ?`, [userId]);
    ...
    getConditionInformation: async(userId)=>{
        return db.query(`SELECT * FROM condition_information WHERE userId = ?`, [userId]);

    },

    getPoliceInformation: async(userId)=>{
        return db.query(`SELECT * FROM police_information WHERE userId = ?`, [userId]);

    },


    getDamageProperty: async(userId)=>{
        return db.query(`SELECT * FROM damage_to_vehicle_property WHERE userId = ?`, [userId]);
    },

    
    getDriverStatement: async(userId)=>{
        return db.query(`SELECT * FROM driver_statement WHERE userId = ?`, [userId]);
    },
